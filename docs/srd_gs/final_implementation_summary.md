...
2. Accepted GT geometry is still unavailable for reflective-region Chamfer, F-score, normal MAE, and depth error. The `ball/points3d.ply` candidate is rejected as accepted GT because it can be generated by the Blender reader random point-cloud path.
...
2. Provide an external accepted GT geometry source or a documented dataset protocol; keep raw-coordinate metrics primary and reject generated `points3d.ply` by default.
...
Fresh verification in Milestone 12:
