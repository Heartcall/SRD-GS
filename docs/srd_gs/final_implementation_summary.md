...
1. Render/GT export is missing for PSNR, SSIM, LPIPS, and reflective-region rendering metrics.
2. Accepted GT geometry loading/evaluation is missing for reflective-region Chamfer, F-score, normal MAE, and depth error.
...
1. Add render-pair export for train/test cameras so `eval_reflective_assets.py` can compute PSNR, SSIM, LPIPS, and reflective-region variants.
2. Add accepted GT geometry loader for the selected synthetic dataset and keep raw-coordinate metrics primary.
...
5. Only after the above gates pass, launch multi-scene ablations from `configs/srd_gs/*.yaml`.

## Verification Status

Fresh verification in Milestone 10:

- `conda run -n ref_gs python -m unittest discover -s tests`: passed, 39 tests.
- `conda run -n ref_gs python -m py_compile ...`: passed for the modified core modules and SRD scripts.
