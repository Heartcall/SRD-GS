...
Test one bounded single-scene control for the plausible opacity/reflection-feature drift mechanism identified in Milestone 23. The control freezes SRD reflection-feature and specular-weight optimizer groups while keeping the M21 neutral-render-gate 300-iteration setup. It is not a broad paper-scale experiment.

## Claim Boundary

- Allowed claim: optimizer-level freezing can control reflection-feature/specular-weight checkpoint drift in the bounded `ball` 300-iteration SRD branch-raster path.
- Allowed claim: the train -> mesh -> texture -> render_eval_pairs -> accepted-GT eval chain still executes with the freeze control.
- Disallowed claim: SRD-GS improves over Ref-GS.
- Disallowed claim: the rendering regression is fixed.
...
- Added `--srd_reflection_feature_lr_scale` and `--srd_specular_weight_lr_scale` to `OptimizationParams`, both defaulting to `1.0`.
- Applied the scales only to the `reflection_feature` and `specular_weight` optimizer groups in `scene/gaussian_model.py`.
...
bash scripts/srd_gs/run_branch_raster_smoke_one_scene.sh \
  --config configs/srd_gs/full_srd_gs_branch_raster_reflection_freeze_i300.yaml \
  --scene_path "/data/liuly/dataset/3DGS/Shiny Blender Synthetic/ball" \
  --output_root outputs/srd_gs_reflection_freeze_m24_i300_dryrun \
  --scene_name ball \
  --iterations 300 \
  --max_mesh_views 4 \
  --depth_trunc 10.0 \
...
bash scripts/srd_gs/run_branch_raster_smoke_one_scene.sh \
  --config configs/srd_gs/full_srd_gs_branch_raster_reflection_freeze_i300.yaml \
  --scene_path "/data/liuly/dataset/3DGS/Shiny Blender Synthetic/ball" \
  --output_root outputs/srd_gs_reflection_freeze_m24_i300 \
  --scene_name ball \
  --iterations 300 \
  --max_mesh_views 4 \
  --depth_trunc 10.0 \
...
- `outputs/srd_gs_reflection_freeze_m24_i300/results/ball/full_srd_gs_branch_raster_reflection_freeze_i300/mesh_surface.ply`
- `outputs/srd_gs_reflection_freeze_m24_i300/results/ball/full_srd_gs_branch_raster_reflection_freeze_i300/pbr_textures_specular_free/baking_report.json`
- `outputs/srd_gs_reflection_freeze_m24_i300/results/ball/full_srd_gs_branch_raster_reflection_freeze_i300/render_eval_pairs/render_eval_manifest.json`
...
The freeze control hit its intended target: reflection-feature drift and specular-weight drift were suppressed. Rendering did not recover; PSNR is lower than M20/M21 and remains far below M18. Refl-PSNR, Chamfer, Normal MAE, and leakage improve relative to M20/M21, but F-score remains zero and the evidence is still one scene only.

Opacity drift is now the more plausible next bounded control target because M24 shows larger activated-opacity mean drift than M20/M21 despite controlling reflection/specular drift.

## Failure Conditions

- If freeze flags leak into render/texture/eval commands, the milestone fails. Dry-run command files show they do not leak.
- If `--enable_srd_gs=False` behavior changes, the milestone fails. Defaults remain neutral at `1.0`, and SRD flags are opt-in.
- If rendering recovery is claimed from these metrics, the milestone fails. Rendering recovery remains NO-GO.
...
- Dry-run command contract passed for `outputs/srd_gs_reflection_freeze_m24_i300_dryrun`.
- Runtime chain passed under `outputs/srd_gs_reflection_freeze_m24_i300` after host-visible CUDA approval.
...
Milestone 25 should be one bounded opacity-drift control for `ball`, dry-run first, with baseline-compatible opt-in flags. Do not launch broad paper-scale experiments until the single-scene checkpoint dynamics and rendering recovery blockers are resolved.
